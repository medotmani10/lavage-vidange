...
-- For Supabase, we use a different approach
-- Create the function but don't attach it to auth.users directly
-- Instead, users should be created via the application or Supabase dashboard
...
-- Alternative: Use Supabase Auth Hooks via Dashboard
-- Go to Authentication -> Hooks -> User Created
-- Add this SQL as a hook:
-- SELECT create_user_record(NEW.id, NEW.email, NEW.raw_user_meta_data->>'full_name', (NEW.raw_user_meta_data->>'role')::user_role, NEW.raw_user_meta_data->>'phone');
